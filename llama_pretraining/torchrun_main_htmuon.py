...
from muon import MuonWithAuxAdam,Muon2WithAuxAdam,Muon3WithAuxAdam, MuonOrthWithAuxAdam, MuonHTWithAuxAdam,MuonGWithAuxAdam, RNNP
...
def main(args):
    ...
    if args.use_modulewise_wd:
        ...
        # --- Explicitly specify 'body/head/embed' ---
        body_modules = nn.ModuleDict({
            "layers": model.model.layers,  # transformer blocks
            "norm": model.model.norm,      # final RMSNorm (1D weights only; will go to the Adam group)
        })
        head_module = model.lm_head
        embed_module = model.model.embed_tokens

        # Are embeddings tied? (some configs share weights between lm_head and embed)
        tied = (
        ...
        # 1) Muon: include only "hidden 2D+ weights" (exclude LoRA small matrices and embeddings)
        hidden_weights = []
        ...
        # 2) Adam: gains/biases (LayerNorm weights are 1D) + non-hidden parts (lm_head and embeddings)
        hidden_gains_biases = [p for _, p in body_modules.named_parameters() if p.ndim < 2]

        if tied:
            # If tied, add only once to avoid "parameter appears in multiple parameter groups" error
        ...
        #        if isinstance(g, dict):
         #           g.setdefault("use_muon", False)   # 没填就默认 False
        #            assert "params" in g
        #            normed.append(g)
        #        else:
        #            # 裸 iterable => 包装成一个 Adam 组（默认 use_muon=False）
        #            normed.append({"params": list(g), "use_muon": False})
        #    return normed

        #trainable_params = normalize_groups(trainable_params)
        ...
        # --- Collect base parameter sets ---

        # 1) hidden_weights: 2D+ tensors in the body (no lora, no embeddings, no lm_head)
        hidden_weights = []
        for name, p in body_modules.named_parameters():
            if (
                p.ndim >= 2
        ...
        # 2) hidden_gains_biases: 1D stuff (norm scales, biases, etc.)
        hidden_gains_biases = [
            p for _, p in body_modules.named_parameters()
            if p.ndim < 2
        ]

        # 3) nonhidden_params: lm_head + embeddings (handle tied so we don't double-add)
        if tied:
            nonhidden_params = list(embed_module.parameters())
        else:
            nonhidden_params = [
        ...
        #
        # Goal:
        #   - Muon group gets BOTH hidden_weights and nonhidden_params
        #   - Adam group gets ONLY hidden_gains_biases
        #
        # We also want to avoid accidentally passing the same tensor in both groups,
        # and also avoid duplicates inside a group.

        # Helper to unique-preserve-order by tensor id()
    ...
    #print(trainable_params[1]['use_muon'])
#print(trainable_params[0]['use_muon'])
   # exit()
    ...
    elif  args.optimizer.lower() == "mlast_m":   
        optimizer = MuonWithAuxAdam(trainable_params)
    ...
    elif  args.optimizer.lower() == "mlast_mg":   
        optimizer = MuonGWithAuxAdam(trainable_params,args.power)
