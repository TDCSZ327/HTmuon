...
class SOAP(optim.Optimizer):
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                if "exp_avg" not in state:
                    ...
                    ### MODIFIED: keep optimizer stats in fp32 for stability
                ...
                ### MODIFIED: match grad_projected dtype to exp_avg (fp32 now)
                ...
                ### MODIFIED: cast update back to param dtype (bf16)
    ...
    def init_preconditioner(self, grad, state, precondition_frequency=10, 
        ...
        if grad.dim() == 1:
            if not precondition_1d or grad.shape[0] > max_precond_dim:
                ...
                ### MODIFIED: GG stored in fp32 for stability
    ...
    def project(self, grad, state, merge_dims=False, max_precond_dim=10000):
        ...
        for mat in state['Q']:
            if len(mat) > 0:
                ### MODIFIED: match grad dtype to mat (Q is fp32)
    ...
    def update_preconditioner(self, grad, state, 
        ...
        if grad.dim() == 1:
            if precondition_1d and grad.shape[0] <= max_precond_dim:
                ...
                ### MODIFIED: cast outer_product to GG dtype before lerp_
                outer_product = outer_product.to(state['GG'][0].dtype)
    ...
    def get_orthogonal_matrix(self, mat):
        ...
        for m in mat:
            if len(m) == 0:
                matrix.append([])
                continue
            ### MODIFIED: always compute in float32
        ...
        for m in matrix:
            ...
            ### MODIFIED: keep Q in float32, don't cast back to bf16
            Q = Q.float()
            final.append(Q)
        return final
    ...
    def get_orthogonal_matrix_QR(self, state, max_precond_dim=10000, merge_dims=False):
        ...
        for m,o in zip(precond_list, orth_list):
            if len(m) == 0:
                ...
                orth_matrix.append([])
                continue
            ### MODIFIED: force float32 for stability
            matrix.append(m.data.float())
